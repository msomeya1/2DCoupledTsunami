program Eulerian
    ...
    character*128, parameter :: mode = 'E', title = 'E_T5000s'
    ! 'E' : Eulerian formulation. 'LD' : Lotto & Dunham (2015) formulation
    ...
    real(8) :: QQw, QQs, t, stime, sdrop, Hscale_w, Hscale_s, g, rho0, Hscale
    real(8) :: Uz_ij, UZ_l, Uz_r, rho_bar, rho_bar_l, rho_bar_r
    real(8) :: dxUx_ij, dxUz_ij, dzUx_ij, dzUz_ij, rho_dxUx_ij, rho_dxUx_l, rho_dxUx_r
    real(8) :: dxUx_l, dxUx_r, dxUz_l, dxUz_r, dzUx_l, dzUx_r, dzUz_l, dzUz_r
    real(8) :: accelx, accelx_l, accelx_r, accelz, accelz_l, accelz_r
    real(8) :: dxSxx_l, dxSxz_l, dzSxz_l, dzSzz_l
    real(8) :: dxSxx_r, dxSxz_r, dzSxz_r, dzSzz_r
    ...
    if ( mode == 'E' ) then
        g = g0
        write(6, *) 'This is Eulerian formulation (symmetric grid system),'
        write(6, *) 'considering gravity in the whole medium.'
    else if ( mode == 'LD' ) then
        g = 0.0
        write(6, *) 'This is Lotto and Dunham (2015) formulation (symmetric grid system),'
        write(6, *) 'considering gravity at the sea surface only.'
    else
        write(6, *) 'Please specify mode correctly (`E` or `LD`).'
        stop
    end if
    ...
    do it = 0, Nt
        ...
        do j = 1, Nz-Nz_pml
            do i = Nx_pml+1, Nx-Nx_pml+1
                if ( j <= JSF ) then 
                    rho0 = rhow_c(j)
                    Hscale = Hscale_w
                else if ( j>= JSF+1 ) then
                    rho0 = rhos_c(j)
                    Hscale = Hscale_s
                end if

                Uz_ij = ( Uz(i, j) + Uz(i, j-1) ) / 2.0
                rho_(i, j) = - rho0 * ( dxUx(i, j) + dzUz(i, j) ) - rho0 * Uz_ij / Hscale
            end do
        end do

        !--------------------  strain -> density (PML left and right side) -------------------- !
        do j = 1, Nz-Nz_pml
            do ipml = 1, Nx_pml
                ...
                Uz_l = ( Uz(il , j) + Uz(il , j-1) ) / 2.0
                Uz_r = ( Uz(ir1, j) + Uz(ir1, j-1) ) / 2.0

                dxUx_l = Gx_c(1, ipml) * dxUx(il , j) + Gx_c(2, ipml) * axUx_l(il , j) 
                dxUx_r = Gx_c(1, ipml) * dxUx(ir1, j) + Gx_c(2, ipml) * axUx_r(ir1, j)

                rho_(il , j) = - rho0 * ( dxUx_l + dzUz(il , j) ) - rho0 * Uz_l / Hscale
                rho_(ir1, j) = - rho0 * ( dxUx_r + dzUz(ir1, j) ) - rho0 * Uz_r / Hscale
            end do
        end do


        !--------------------  strain -> density (PML bottom) -------------------- !
        do jpml = 1, Nz_pml
            j = Nz-Nz_pml+jpml
            do i = Nx_pml+1, Nx-Nx_pml+1
                Uz_ij = ( Uz(i, j) + Uz(i, j-1) ) / 2.0
                dzUz_ij = Gz_c(1, jpml) * dzUz(i, j) + Gz_c(2, jpml) * azUz(i, j)
                rho_(i, j) = - rhos_c(j) * ( dxUx(i, j) + dzUz_ij ) - rhos_c(j) * Uz_ij / Hscale_s
            end do
        end do

        !--------------------  strain -> density (PML bottomleft and bottomright corner) -------------------- !
        do jpml = 1, Nz_pml
            j = Nz-Nz_pml+jpml
            do ipml = 1, Nx_pml
                il = Nx_pml+1-ipml ; ir = Nx-Nx_pml+ipml ; ir1 = ir+1
                ...
                rho_(il , j) = - rhos_c(j) * ( dxUx_l + dzUz_l ) - rhos_c(j) * Uz_l / Hscale_s
                rho_(ir1, j) = - rhos_c(j) * ( dxUx_r + dzUz_r ) - rhos_c(j) * Uz_r / Hscale_s
            end do
        end do
        ...
        do j = 1, JSF
            do i = Nx_pml+1, Nx-Nx_pml+1
                Uz_ij = ( Uz(i, j) + Uz(i, j-1) ) / 2.0
                P(i, j) = - rhow_c(j) * Cw**2 * ( dxUx(i, j) + dzUz(i, j) ) - rhow_c(j) * g * Uz_ij
            end do
        end do
        ...
        do j = 1, JSF
            do ipml = 1, Nx_pml
                ...
                P(il , j) = - rhow_c(j) * Cw**2 * ( dxUx_l + dzUz(il , j) ) - rhow_c(j) * g * Uz_l
                P(ir1, j) = - rhow_c(j) * Cw**2 * ( dxUx_r + dzUz(ir1, j) ) - rhow_c(j) * g * Uz_r
            end do
        end do
        ...
        do j = JSF+1, Nz-Nz_pml
            do i = Nx_pml+1, Nx-Nx_pml
                ...
                Sxx(i, j) = ( Vp**2 * dxUx(i, j) + Vps2 * dzUz(i, j) ) * rhos_c(j) + rhos_c(j) * g * Uz_ij
                Szz(i, j) = ( Vp**2 * dzUz(i, j) + Vps2 * dxUx(i, j) ) * rhos_c(j) + rhos_c(j) * g * Uz_ij
        ...
        do j = JSF+1, Nz-Nz_pml
            do ipml = 1, Nx_pml
                ...
                Sxx(il, j) = ( Vp**2 * dxUx_l + Vps2 * dzUz(il, j) ) * rhos_c(j) + rhos_c(j) * g * Uz_l
                Szz(il, j) = ( Vp**2 * dzUz(il, j) + Vps2 * dxUx_l ) * rhos_c(j) + rhos_c(j) * g * Uz_l
                Sxz(il, j) = Vs**2 * ( dxUz_l + dzUx(il, j) ) * rhos_b(j) 

                Sxx(ir1, j) = ( Vp**2 * dxUx_r + Vps2 * dzUz(ir1, j) ) * rhos_c(j) + rhos_c(j) * g * Uz_r
                Szz(ir1, j) = ( Vp**2 * dzUz(ir1, j) + Vps2 * dxUx_r ) * rhos_c(j) + rhos_c(j) * g * Uz_r
        ...
        do jpml = 1, Nz_pml
            ...
            do i = Nx_pml+1, Nx-Nx_pml
                ...
                Sxx(i, j) = ( Vp**2 * dxUx(i, j) + Vps2 * dzUz_ij ) * rhos_c(j) + rhos_c(j) * g * Uz_ij
                Szz(i, j) = ( Vp**2 * dzUz_ij + Vps2 * dxUx(i, j) ) * rhos_c(j) + rhos_c(j) * g * Uz_ij
        ...
        do jpml = 1, Nz_pml
            ...
            do ipml = 1, Nx_pml
                ...
                Sxx(il, j) = ( Vp**2 * dxUx_l + Vps2 * dzUz_l ) * rhos_c(j) + rhos_c(j) * g * Uz_l
                Szz(il, j) = ( Vp**2 * dzUz_l + Vps2 * dxUx_l ) * rhos_c(j) + rhos_c(j) * g * Uz_l
                Sxz(il, j) = Vs**2 * ( dxUz_l + dzUx_l ) * rhos_b(j) 

                Sxx(ir1, j) = ( Vp**2 * dxUx_r + Vps2 * dzUz_r ) * rhos_c(j) + rhos_c(j) * g * Uz_r
                Szz(ir1, j) = ( Vp**2 * dzUz_r + Vps2 * dxUx_r ) * rhos_c(j) + rhos_c(j) * g * Uz_r
        ...
        !------------------------------ sea surface (j = 0) ------------------------------!
        do i = 1, Nx+1
            rho_bar = 1.5 * rho_(i, 1) - 0.5 * rho_(i, 2)
            accelz = ( -2 * P(i, 1) / dz - 2 * rhow_0 * g0 * Uz(i, 0) / dz + rho_bar * g ) / ( rhow_0 + rhoa )
        ...
        do j = 1, JSF-1
            do i = Nx_pml+1, Nx-Nx_pml
                rho_bar = ( rho_(i, j) + rho_(i, j+1) ) / 2.0
                accelx = - dxP(i, j) / rhow_c(j)
                accelz = ( - dzP(i, j) + g * rho_bar ) / rhow_b(j)
        ...
        do j = 1, JSF-1
            do ipml = 1, Nx_pml
                ...
                rho_bar_l = ( rho_(il , j) + rho_(il , j+1) ) / 2.0
                rho_bar_r = ( rho_(ir1, j) + rho_(ir1, j+1) ) / 2.0

                accelx_l = - ( Gx_b(1, ipml) * dxP(il, j) + Gx_b(2, ipml) * axP_l(il, j) ) / rhow_c(j)
                accelx_r = - ( Gx_b(1, ipml) * dxP(ir, j) + Gx_b(2, ipml) * axP_r(ir, j) ) / rhow_c(j)
 
                accelz_l = ( - dzP(il , j) + g * rho_bar_l ) / rhow_b(j)
                accelz_r = ( - dzP(ir1, j) + g * rho_bar_r ) / rhow_b(j)
        ...
        !------------------------------ seafloor (j = JSF) ------------------------------!
        do i = Nx_pml+1, Nx-Nx_pml
            accelx = - dxP(i, JSF) / rhow_c(JSF)
            rho_bar = 1.5 * ( rho_(i, JSF+1) + rho_(i, JSF) ) - 0.5 * ( rho_(i, JSF+2) + rho_(i, JSF-1) )
            accelz = ( &
            &   2 * ( Szz(i, JSF+1) + P(i, JSF) - (rhos_0 - rhow_b(JSF)) * g * Uz(i, JSF) ) / dz  &
            &   + rho_bar * g ) / ( rhos_0 + rhow_b(JSF) )
        ...
        do ipml = 1, Nx_pml
            ...
            accelx_l = - ( Gx_b(1, ipml) * dxP(il, JSF) + Gx_b(2, ipml) * axP_l(il, JSF) ) / rhow_c(JSF)
            accelx_r = - ( Gx_b(1, ipml) * dxP(ir, JSF) + Gx_b(2, ipml) * axP_r(ir, JSF) ) / rhow_c(JSF)

            rho_bar_l = 1.5 * ( rho_(il , JSF+1) + rho_(il , JSF) ) - 0.5 * ( rho_(il , JSF+2) + rho_(il , JSF-1) )
            rho_bar_r = 1.5 * ( rho_(ir1, JSF+1) + rho_(ir1, JSF) ) - 0.5 * ( rho_(ir1, JSF+2) + rho_(ir1, JSF-1) )

            accelz_l = ( &
            &   2 * ( Szz(il, JSF+1) + P(il, JSF) - (rhos_0 - rhow_b(JSF)) * g * Uz(il, JSF) ) / dz  &
            &   + rho_bar_l * g ) / ( rhos_0 + rhow_b(JSF) )

            accelz_r = ( &
            &   2 * ( Szz(ir1, JSF+1) + P(ir1, JSF) - (rhos_0 - rhow_b(JSF)) * g * Uz(ir1, JSF) ) / dz  &
            &   + rho_bar_r * g ) / ( rhos_0 + rhow_b(JSF) )
        ...
        do j = JSF+1, Nz-Nz_pml
            do i = Nx_pml+1, Nx-Nx_pml
                ...
                accelx = ( dxSxx(i, j) + dzSxz(i, j) ) / rhos_c(j)
                accelz = ( dxSxz(i, j) + dzSzz(i, j) + rho_bar * g ) / rhos_b(j)
        ...
        do j = JSF+1, Nz-Nz_pml
            do ipml = 1, Nx_pml
                ...
                accelx_l = ( dxSxx_l + dzSxz(il, j) ) / rhos_c(j)
                accelx_r = ( dxSxx_r + dzSxz(ir, j) ) / rhos_c(j)

                accelz_l = ( dxSxz_l + dzSzz(il , j) + rho_bar_l * g ) / rhos_b(j)
                accelz_r = ( dxSxz_r + dzSzz(ir1, j) + rho_bar_r * g ) / rhos_b(j)
        ...
        do jpml = 1, Nz_pml
            ...
            do i = Nx_pml+1, Nx-Nx_pml
                ...
                if ( j == Nz ) then
                    rho_bar = 1.5 * rho_(i, Nz) -0.5 * rho_(i, Nz-1)
                else 
                    rho_bar = ( rho_(i, j) + rho_(i, j+1) ) / 2.0
                end if
                ...
                accelx = ( dxSxx(i, j) + dzSxz_ij ) / rhos_c(j)
                accelz = ( dxSxz(i, j) + dzSzz_ij + rho_bar * g ) / rhos_b(j)
        ...
        do jpml = 1, Nz_pml
            ...
            do ipml = 1, Nx_pml
                ...
                if ( j == Nz ) then
                    rho_bar_l = 1.5 * rho_(il , Nz) -0.5 * rho_(il , Nz-1)
                    rho_bar_r = 1.5 * rho_(ir1, Nz) -0.5 * rho_(ir1, Nz-1)
                else
                    rho_bar_l = ( rho_(il , j) + rho_(il , j+1) ) / 2.0
                    rho_bar_r = ( rho_(ir1, j) + rho_(ir1, j+1) ) / 2.0
                end if
                ...
                accelx_l = ( dxSxx_l + dzSxz_l ) / rhos_c(j)
                accelx_r = ( dxSxx_r + dzSxz_r ) / rhos_c(j)

                accelz_l = ( dxSxz_l + dzSzz_l + rho_bar_l * g ) / rhos_b(j)
                accelz_r = ( dxSxz_r + dzSzz_r + rho_bar_r * g ) / rhos_b(j)
