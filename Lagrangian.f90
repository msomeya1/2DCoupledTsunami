program Lagrangian
    ...
    character*128, parameter :: title = 'L_T5000s'
    ...
    write(6, *) 'This is Lagrangian formulation (symmetric grid system).'
    ...
    do it = 0, Nt
        ...
        do i = Nx_pml+1, Nx-Nx_pml+1
            ! dxUx_ij = dxUx(i, 1)
            dxUx_ij = 1.5 * dxUx(i, 1) -0.5 * dxUx(i, 2)
            accelz = ( -2 * P(i, 1) / dz - g * rhow_0 * dxUx_ij ) / ( rhow_0 + rhoa )
        ...
        do ipml = 1, Nx_pml
            ...
            ! dxUx_l = Gx_c(1, ipml) * dxUx(il , 1) + Gx_c(2, ipml) * axUx_l(il , 1)
            ! dxUx_r = Gx_c(1, ipml) * dxUx(ir1, 1) + Gx_c(2, ipml) * axUx_r(ir1, 1)
            dxUx_l = Gx_c(1, ipml) * ( 1.5 *   dxUx(il , 1) -0.5 *   dxUx(il , 2) ) &
                 & + Gx_c(2, ipml) * ( 1.5 * axUx_l(il , 1) -0.5 * axUx_l(il , 2) )
            dxUx_r = Gx_c(1, ipml) * ( 1.5 *   dxUx(ir1, 1) -0.5 *   dxUx(ir1, 2) ) &
                 & + Gx_c(2, ipml) * ( 1.5 * axUx_r(ir1, 1) -0.5 * axUx_r(ir1, 2) )

            accelz_l = ( -2 * P(il , 1) / dz - g * rhow_0 * dxUx_l ) / ( rhow_0 + rhoa )
            accelz_r = ( -2 * P(ir1, 1) / dz - g * rhow_0 * dxUx_r ) / ( rhow_0 + rhoa )
        ...
        do j = 1, JSF-1
            do i = Nx_pml+1, Nx-Nx_pml
                dxUz_ij = ( dxUz(i, j) + dxUz(i, j-1) ) / 2.0 
                dxUx_ij = ( dxUx(i, j+1) + dxUx(i, j) ) / 2.0

                accelx = -dxP(i, j) / rhow_c(j) + g * dxUz_ij
                accelz = -dzP(i, j) / rhow_b(j) - g * dxUx_ij
        ...
        do j = 1, JSF-1
            do ipml = 1, Nx_pml
                ...
                dxUz_l = Gx_b(1, ipml) * (   dxUz(il, j) +   dxUz(il, j-1) ) / 2.0 &
                     & + Gx_b(2, ipml) * ( axUz_l(il, j) + axUz_l(il, j-1) ) / 2.0
                dxUz_r = Gx_b(1, ipml) * (   dxUz(ir, j) +   dxUz(ir, j-1) ) / 2.0 &
                     & + Gx_b(2, ipml) * ( axUz_r(ir, j) + axUz_r(ir, j-1) ) / 2.0

                dxUx_l = Gx_c(1, ipml) * (   dxUx(il , j+1) +   dxUx(il , j  ) ) / 2.0 &
                     & + Gx_c(2, ipml) * ( axUx_l(il , j+1) + axUx_l(il , j  ) ) / 2.0
                dxUx_r = Gx_c(1, ipml) * (   dxUx(ir1, j+1) +   dxUx(ir1, j  ) ) / 2.0 &
                     & + Gx_c(2, ipml) * ( axUx_r(ir1, j+1) + axUx_r(ir1, j  ) ) / 2.0

                accelx_l = - ( Gx_b(1, ipml) * dxP(il, j) + Gx_b(2, ipml) * axP_l(il, j) ) / rhow_c(j) + g * dxUz_l
                accelz_l = - dzP(il, j) / rhow_b(j) - g * dxUx_l 

                accelx_r = - ( Gx_b(1, ipml) * dxP(ir, j) + Gx_b(2, ipml) * axP_r(ir, j) ) / rhow_c(j) + g * dxUz_r
                accelz_r = - dzP(ir1, j) / rhow_b(j) - g * dxUx_r 
        ...
        !------------------------------ seafloor (j = JSF) ------------------------------!
        do i = Nx_pml+1, Nx-Nx_pml
            ! dxUz_ij = ( dxUz(i, JSF) + dxUz(i, JSF-1) ) / 2.0
            dxUz_ij = -0.5 * dxUz(i, JSF-2) + 1.5 * dxUz(i, JSF-1)
            accelx = - dxP(i, JSF) / rhow_c(JSF) + g * dxUz_ij
            
            rho_dxUx_ij = rhow_b(JSF) * ( 1.5 * dxUx(i, JSF  ) -0.5 * dxUx(i, JSF-1) ) &
            &           + rhos_0      * ( 1.5 * dxUx(i, JSF+1) -0.5 * dxUx(i, JSF+2) ) 
            accelz = ( 2 * ( Szz(i, JSF+1) + P(i, JSF) ) / dz - g * rho_dxUx_ij ) / ( rhos_0 + rhow_b(JSF) )

            ! accelz = ( &
            ! &     2 * ( Szz(i, JSF+1) + P(i, JSF) ) / dz                     &
            ! &   - g * ( rhos * dxUx(i, JSF+1) + rhow_c(JSF) * dxUx(i, JSF) ) & 
            ! & ) / ( rhos + rhow_b(JSF) )
        ...
        do ipml = 1, Nx_pml
            ...
            ! dxUz_l = Gx_b(1, ipml) * (   dxUz(il, JSF) +   dxUz(il, JSF-1) ) / 2.0 &
            !      & + Gx_b(2, ipml) * ( axUz_l(il, JSF) + axUz_l(il, JSF-1) ) / 2.0
            ! dxUz_r = Gx_b(1, ipml) * (   dxUz(ir, JSF) +   dxUz(ir, JSF-1) ) / 2.0 &
            !      & + Gx_b(2, ipml) * ( axUz_r(ir, JSF) + axUz_r(ir, JSF-1) ) / 2.0
            dxUz_l = Gx_b(1, ipml) * ( -0.5 *   dxUz(il, JSF-2) + 1.5 *   dxUz(il, JSF-1) ) &
                 & + Gx_b(2, ipml) * ( -0.5 * axUz_l(il, JSF-2) + 1.5 * axUz_l(il, JSF-1) )
            dxUz_r = Gx_b(1, ipml) * ( -0.5 *   dxUz(ir, JSF-2) + 1.5 *   dxUz(ir, JSF-1) ) &
                 & + Gx_b(2, ipml) * ( -0.5 * axUz_r(ir, JSF-2) + 1.5 * axUz_r(ir, JSF-1) )

            accelx_l = - ( Gx_b(1, ipml) * dxP(il, JSF) + Gx_b(2, ipml) * axP_l(il, JSF) ) / rhow_c(JSF) + g * dxUz_l
            accelx_r = - ( Gx_b(1, ipml) * dxP(ir, JSF) + Gx_b(2, ipml) * axP_r(ir, JSF) ) / rhow_c(JSF) + g * dxUz_r


            rho_dxUx_l = rhow_b(JSF) * (                                             &
                Gx_c(1, ipml) * ( 1.5 *   dxUx(il, JSF  ) -0.5 *   dxUx(il, JSF-1) ) &
              + Gx_c(2, ipml) * ( 1.5 * axUx_l(il, JSF  ) -0.5 * axUx_l(il, JSF-1) ) &
            ) + rhos_0 * (                                                           &
                Gx_c(1, ipml) * ( 1.5 *   dxUx(il, JSF+1) -0.5 *   dxUx(il, JSF+2) ) &
              + Gx_c(2, ipml) * ( 1.5 * axUx_l(il, JSF+1) -0.5 * axUx_l(il, JSF+2) ) &
            ) 

            rho_dxUx_r = rhow_b(JSF) * (                                             &
                Gx_c(1, ipml) * ( 1.5 *   dxUx(ir1, JSF  ) -0.5 *   dxUx(ir1, JSF-1) ) &
              + Gx_c(2, ipml) * ( 1.5 * axUx_r(ir1, JSF  ) -0.5 * axUx_r(ir1, JSF-1) ) &
            ) + rhos_0 * (                                                           &
                Gx_c(1, ipml) * ( 1.5 *   dxUx(ir1, JSF+1) -0.5 *   dxUx(ir1, JSF+2) ) &
              + Gx_c(2, ipml) * ( 1.5 * axUx_r(ir1, JSF+1) -0.5 * axUx_r(ir1, JSF+2) ) &
            ) 

            accelz_l = ( 2 * ( Szz(il , JSF+1) + P(il , JSF) ) / dz - g * rho_dxUx_l ) / ( rhos_0 + rhow_b(JSF) )
            accelz_r = ( 2 * ( Szz(ir1, JSF+1) + P(ir1, JSF) ) / dz - g * rho_dxUx_r ) / ( rhos_0 + rhow_b(JSF) )
        ...
        do j = JSF+1, Nz-Nz_pml
            do i = Nx_pml+1, Nx-Nx_pml
                ...
                accelx = ( dxSxx(i, j) + dzSxz(i, j) ) / rhos_c(j) + g * dxUz_ij
                accelz = ( dxSxz(i, j) + dzSzz(i, j) ) / rhos_b(j) - g * dxUx_ij
        ...
        do j = JSF+1, Nz-Nz_pml
            do ipml = 1, Nx_pml
                ...
                accelx_l = ( dxSxx_l + dzSxz(il, j) ) / rhos_c(j) + g * dxUz_l
                accelx_r = ( dxSxx_r + dzSxz(ir, j) ) / rhos_c(j) + g * dxUz_r

                accelz_l = ( dxSxz_l + dzSzz(il , j) ) / rhos_b(j) - g * dxUx_l
                accelz_r = ( dxSxz_r + dzSzz(ir1, j) ) / rhos_b(j) - g * dxUx_r
        ...
        do jpml = 1, Nz_pml
            ...
            do i = Nx_pml+1, Nx-Nx_pml
                ...
                if ( j == Nz ) then
                    dxUx_ij = 1.5 * dxUx(i, Nz) -0.5 * dxUx(i, Nz-1)
                else
                    dxUx_ij = ( dxUx(i, j+1) + dxUx(i, j) ) / 2.0 
                end if
                ...
                accelx = ( dxSxx(i, j) + dzSxz_ij ) / rhos_c(j) + g * dxUz_ij
                accelz = ( dxSxz(i, j) + dzSzz_ij ) / rhos_b(j) - g * dxUx_ij
        ...
        do jpml = 1, Nz_pml
            ...
            do ipml = 1, Nx_pml
                ...
                if ( j == Nz ) then
                    dxUx_l = Gx_c(1, ipml) * ( 1.5 *   dxUx(il , Nz) -0.5 *   dxUx(il , Nz-1) ) &
                           + Gx_c(2, ipml) * ( 1.5 * axUx_l(il , Nz) -0.5 * axUx_l(il , Nz-1) ) 
                    dxUx_r = Gx_c(1, ipml) * ( 1.5 *   dxUx(ir1, Nz) -0.5 *   dxUx(ir1, Nz-1) ) &
                           + Gx_c(2, ipml) * ( 1.5 * axUx_r(ir1, Nz) -0.5 * axUx_r(ir1, Nz-1) )
                else
                    dxUx_l = Gx_c(1, ipml) * (   dxUx(il , j+1) +   dxUx(il , j) ) / 2.0 &
                         & + Gx_c(2, ipml) * ( axUx_l(il , j+1) + axUx_l(il , j) ) / 2.0
                    dxUx_r = Gx_c(1, ipml) * (   dxUx(ir1, j+1) +   dxUx(ir1, j) ) / 2.0 &
                ...
                accelx_l = ( dxSxx_l + dzSxz_l ) / rhos_c(j) + g * dxUz_l
                accelx_r = ( dxSxx_r + dzSxz_r ) / rhos_c(j) + g * dxUz_r

                accelz_l = ( dxSxz_l + dzSzz_l ) / rhos_b(j) - g * dxUx_l
                accelz_r = ( dxSxz_r + dzSzz_r ) / rhos_b(j) - g * dxUx_r
